...
<body class="bg-light">
                    ...
                    <div class="">
                        ...
                        <li class="mx-2 my-1">
                            <a href="{{ route('admin.marketing.metricas') }}"
                                class="nav-link px-3 {{ request()->is(['administrador/marketing/metricas*']) ? 'active-item' : null }} menu">
                                <span class="fw-bold">
                                    <i class="bi bi-radar me-2"></i>
                                </span>
                                <span>Radar Meta</span>
                            </a>
                        </li>
                        <li class="mx-2 my-1">
                            <a href="{{ route('admin.marketing.emails') }}"
                                class="nav-link px-3 {{ request()->is(['administrador/marketing/emails*']) ? 'active-item' : null }} menu">
                                <span class="fw-bold">
    ...
    <main>
                        ...
                        <ul class="dropdown-menu dropdown-menu-end shadow border-0 mt-2 py-0"
                            ...
                            <li>
                                <a class="dropdown-item py-2" href="/">
                                    <i class="bi bi-globe me-2"></i> Ir a la plataforma
                                </a>
                            </li>
                            <li>
        ...
        <div class="mb-3" id="contenido" style="padding-top: 80px;">
    ...
    <script>
        ...
        $('.select2_bootstrap, .select2_bootstrap_2').select2({
            theme: "bootstrap-5",
    ...
    <script>
        $(document).ready(function() {
            ...
            function cargarNotificaciones() {
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        var badges = [];
                        if (data.vencidas > 0) badges.push('<span class="badge bg-danger">' + data.vencidas + ' vencida(s)</span>');
                        if (data.proximas > 0) badges.push('<span class="badge bg-warning text-dark">' + data.proximas + ' por iniciar</span>');
                        if (data.recordatorios > 0) badges.push('<span class="badge bg-info">' + data.recordatorios + ' recordatorio(s)</span>');
                        if (data.seguimientos > 0) badges.push('<span class="badge bg-info">' + data.seguimientos + ' seguimiento(s)</span>');
                        ...
                        $.each(data.items, function(i, item) {
                            ...
                            var cursor = 'style="border-left: 4px solid ' + colors.border + ' !important; background: ' + colors.bg + '; cursor: pointer;"';

                            if (item.descartable) {
                                html += '<div class="px-3 py-2 border-bottom notif-item-clickeable" data-id="' + item.id + '" data-categoria="' + item.categoria + '" data-url="' + item.url + '" ' + (item.prospecto_id ? 'data-prospecto-id="' + item.prospecto_id + '" ' : '') + cursor + '>';
                            } else {
                                html += '<div class="px-3 py-2 border-bottom notif-item-link" data-url="' + item.url + '" ' + cursor + '>';
                            }

                            html += '<div class="d-flex justify-content-between align-items-center mb-1">';
                            html += '<span class="small fw-semibold"><i class="bi ' + item.icono + ' me-1"></i>' + item.tipo_actividad + '</span>';
                            html += '<span class="badge bg-' + item.color + (item.color === 'warning' ? ' text-dark' : '') + '" style="font-size: 0.65rem;">' + item.etiqueta + '</span>';
                            html += '</div>';

                            if (item.categoria !== 'seguimiento') {
                                html += '<div class="small text-truncate mb-1" style="max-width: 220px;">' + item.titulo + '</div>';
                            } else {
                                html += '<div class="small text-truncate mb-1" style="max-width: 220px;">' + item.titulo.replace('Seguimiento: ', '') + '</div>';
                            }

                            html += '<div class="text-muted" style="font-size: 0.75rem;"><i class="bi bi-calendar3 me-1"></i>' + item.fecha + ' · ' + item.tiempo + '</div></div>';
                        });
            ...
            $(document).on('click', '.notif-item-clickeable', function(e) {
                ...
                $item.css({'opacity': '0.5', 'pointer-events': 'none'});

                var payload = { _token: csrfToken, categoria: categoria };
                if (categoria === 'seguimiento') payload.prospecto_id = prospectoId;
                else payload.actividad_id = itemId;

                $.ajax({
                    url: descartarUrl,
                    method: 'POST',
                    data: payload,
                    complete: function() { window.location.href = url; }
                });
            });
    ...
    <script>
        (function() {
            ...
            @endif
            @if (session('error'))
                Toast.fire({ icon: 'error', title: @json(session('error')), timer: 5000 });
            @endif
            @if (session('info'))
