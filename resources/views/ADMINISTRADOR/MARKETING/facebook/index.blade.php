<div class="row mb-4">
    <div class="col-lg-4 mb-4 mb-lg-0">
        <div class="card border-0 shadow-sm rounded-4 h-100" style="border-top: 4px solid #1C3146;">
            <div class="card-header bg-white py-3 border-bottom d-flex justify-content-between align-items-center">
                <h6 class="mb-0 fw-bold" style="color: #1C3146;"><i class="bi bi-star-fill me-2" style="color: #20c997;"></i>Top Leads Facebook</h6>
            </div>
                    ...
                    <div class="d-flex align-items-center mb-3 p-2 border rounded shadow-sm bg-light">
                        <div class="position-relative">
                            ...
                            <span class="position-absolute bottom-0 start-100 translate-middle badge rounded-pill p-1" style="background-color: #1C3146;">
                                <i class="bi bi-facebook" style="font-size: 0.6rem;"></i>
                            </span>
                        </div>
    ...
    <div class="col-lg-8">
        <form action="{{ route('admin.marketing.metricas') }}" method="GET" class="bg-white p-3 rounded-4 shadow-sm border d-flex gap-3 flex-wrap align-items-center mb-4" style="border-left: 4px solid #1C3146 !important;">
            ...
            <div class="flex-grow-1 d-flex gap-3 align-items-center">
                ...
                <div class="form-floating" style="min-width: 130px;">
                    <input type="date" name="fecha_inicio" value="{{ $fechaInicio ?? '' }}" class="form-control rounded-pill auto-submit" id="fbFI" placeholder="Inicio">
                    <label for="fbFI">Fecha Inicio</label>
                </div>
                <div class="form-floating" style="min-width: 130px;">
                    <input type="date" name="fecha_fin" value="{{ $fechaFin ?? '' }}" class="form-control rounded-pill auto-submit" id="fbFF" placeholder="Fin">
                    <label for="fbFF">Fecha Fin</label>
                </div>
                <div class="form-floating flex-grow-1">
                    <input type="text" name="search" value="{{ $search ?? '' }}" class="form-control rounded-pill auto-submit" id="fbSearch" placeholder="Buscar por palabra...">
                    <label for="fbSearch"><i class="bi bi-search me-1"></i> Buscar en Publicaciones FB</label>
                </div>
            </div>
            <div>
        ...
        <div class="card border-0 shadow-sm rounded-4 overflow-hidden">
            <div class="card-header bg-white py-3 border-bottom d-flex justify-content-between align-items-center">
                <h6 class="mb-0 fw-bold" style="color: #1C3146;"><i class="bi bi-facebook me-2"></i>Feed de Publicaciones</h6>
                <span class="badge bg-light border rounded-pill px-3 py-2" style="color: #6c757d;">Mostrando {{ count($fbData['recent_posts'] ?? []) }} posts</span>
            </div>
            <div class="table-responsive" style="max-height: 400px; overflow-y: auto;">
                <table class="table table-hover align-middle mb-0 border-0">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td class="ps-4 py-3">
                                <img src="{{ $post['full_picture'] ?? asset('img/no-image.png') }}" class="rounded-3 shadow-sm border border-light" style="width: 60px; height: 60px; object-fit: cover;">
                            </td>
                            <td>
                                <p class="mb-1 fw-bold text-dark small" style="line-height: 1.4;">{{ Str::limit($post['message'] ?? 'Post sin texto', 100) }}</p>
                                <small style="color: #6c757d;"><i class="bi bi-calendar-check me-1"></i>{{ \Carbon\Carbon::parse($post['created_time'])->translatedFormat('d M - h:i A') }}</small>
                            </td>
                            ...
                            <td class="text-center pe-4">
                                @php 
                                    $cCount = collect($post['comments']['data'] ?? [])->reduce(function($total, $c) {
                                        return $total + 1 + count($c['comments']['data'] ?? []);
                                    }, 0);
                                    $hasReplies = collect($post['comments']['data'] ?? [])->contains(function($c) {
                                        return (isset($c['comments']['data']) && count($c['comments']['data']) > 0);
                                    });
                                @endphp
                                @if($hasReplies)
                                    <button class="btn btn-sm rounded-pill fw-bold shadow-sm px-3 py-1" style="background-color: #20c997; color: white; border: none;" onclick="openCommentModal({{ json_encode($post) }}, 'fb')">
                                        <i class="bi bi-chat-text-fill"></i> <span id="badge-count-{{ $post['id'] }}">{{ $cCount }}</span>
                                    </button>
                                @elseif($cCount > 0)
                                    <button class="btn btn-sm rounded-pill fw-bold shadow-sm px-3 py-1" style="background-color: #1C3146; color: white; border: none;" onclick="openCommentModal({{ json_encode($post) }}, 'fb')">
                                        <i class="bi bi-chat-text-fill"></i> <span id="badge-count-{{ $post['id'] }}">{{ $cCount }}</span>
                                    </button>
                                @else
                                    <button class="btn btn-sm btn-light border rounded-pill fw-bold shadow-sm px-3 py-1" style="color: #6c757d;" onclick="openCommentModal({{ json_encode($post) }}, 'fb')">
                                        <i class="bi bi-chat-text-fill"></i> <span id="badge-count-{{ $post['id'] }}">{{ $cCount }}</span>
                                    </button>
                                @endif
                                <a href="{{ $post['permalink'] ?? '#' }}" target="_blank" class="btn btn-sm btn-light border rounded-circle ms-1" style="color: #6c757d;" title="Ver en Meta">
                        ...
                        <tr>
                            <td colspan="4" class="text-center py-5" style="color: #6c757d;">
                                <i class="bi bi-file-earmark-post fs-2 d-block mb-2 opacity-25"></i>
