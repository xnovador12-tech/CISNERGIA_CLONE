...
<head>
    ...
    <title>VIKINGOS | MOVIMIENTO DE SALIDA - {{ $admin_salida->codigo }} - PDF</title>
...
<body>
                        ...
                        <table class="w-100 mt-3">
                            <tr>
                                <td style="width: 45%" class="border-0 border-bottom border-1 border-dark">
                                    <p class="text-uppercase mb-2 text-start fw-bold">Movimiento de salida</p>
                                </td>
    ...
    <div class="contenido">
        <table class="mt-2 mb-3" style="width: 100%; font-size: 10px">
            <thead class="bg-encabezado">
                <tr>
                    ...
                    <th class="text-uppercase border border-encabezado text-center py-1 fw-bold small" style=" width: 35%">Requerimiento de Origen</th>
                </tr>
            </thead>
            <tbody class="bg-cuerpo">
                @php
                    $sucursal_almacen = \App\Models\Sucursal::where('id',$admin_salida->almacene->sucursal_id)->first();
                @endphp
                <tr>
                    <td class="text-uppercase border border-encabezado text-center py-2 fw-normal small">
                        {{ $admin_salida->codigo }}
                    </td>
                    <td class="text-uppercase border border-encabezado text-center py-2 fw-normal small">{{ $admin_salida->motivo }}</td>
                    <td class="text-uppercase border border-encabezado text-center py-2 fw-normal small">{{ \Carbon\Carbon::parse($admin_salida->created_at)->format('d-m-Y') }}</td>
                    <td class="text-uppercase border border-encabezado text-center py-2 fw-normal small">{{$admin_salida->almacene->nombre}}</td>
                    <td class="text-uppercase border border-encabezado text-center py-2 fw-normal small">{{$admin_salida->codigo_reqcompra}}</td>
                </tr>
            </tbody>
        </table>
        ...
        <table class="my-2" style="width: 100%; font-size: 10px;">
            ...
            <tbody class="bg-cuerpo">
                <tr>
                    <td class="text-uppercase border border-encabezado text-center py-2 fw-normal small">{{ $admin_salida->registrado_por }}</td>
                </tr>
            </tbody>
        </table>
            ...
            <thead class="">
                ...
                <tbody>
                    ...
                    @foreach($dtlle_salida as $item)
                        @php
                            $tipo_ = \App\Models\Tipo::where('nombre',$item->tipo_producto)->first();
                        @endphp
                        <tr class="text-uppercase">
                            <td class="border-0 border-end border-start py-1 border-encabezado text-center">{{ $contador }}</td>
                            <td class="border-0 border-end py-1 border-encabezado text-center">{{ $tipo_->nombre }}</td>
                    ...
                    @endfor
                        <tr class="text-uppercase">
                            ...
                            <td class="border-0 border-top border-end border-bottom  py-1 border-encabezado text-end px-2 fw-bold">{{ number_format($admin_salida->total ?? 0, 2, '.', ',') }}</td>
                        </tr>
                </tbody>
        </table>
        ...
        <div class="card border-encabezado mt-3 no-page-break" style="font-size: 11px;">
            <div class="card-body p-1">
                ...
                <p>- {{$admin_salida->descripcion?$admin_salida->descripcion:'Sin observaciones'}}</p>
            </div>
        </div>
    </div>
