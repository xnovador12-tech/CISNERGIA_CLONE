...
@endphp
<div class="modal fade" id="showmodulo_solar{{$sede->id}}" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-xl modal-dialog-scrollable">
        <div class="modal-content">
            <div class="modal-header bg-secondary text-white py-2">
                <span class="modal-title text-uppercase small" id="staticBackdropLabel">Almacén de modulo solar - Sede {{Auth::user()->persona->sede?Auth::user()->persona->sede->name:'General'}}</span>
            ...
            <div class="modal-body">
                ...
                <table id="" class="display table table-hover table-sm py-2" cellspacing="0" style="width:100%">
                    ...
                    <tbody id="inventario_table">
                        @foreach ($al_modulo_solar as $al_modulo_solars)
                            @php
                            ...
                            <tr class="">
                                ...
                                <td class="fw-normal align-middle text-uppercase small">{{ $al_modulo_solars->producto }}</td>
                                <td class="fw-normal align-middle">{{ $al_modulo_solars->umedida }}</td>
                                <td class="fw-normal align-middle">
                                    @if($al_modulo_solars->cantidad <= 10)
                                        <span class="badge w-100 bg-danger">{{ $al_modulo_solars->cantidad }}</span>
                                    @elseif($al_modulo_solars->cantidad <= 20)
                                        <span class="badge w-100 bg-warning">{{ $al_modulo_solars->cantidad }}</span>
                                    @elseif($al_modulo_solars->cantidad >= 21)
                                        <span class="badge w-100 bg-success">{{ $al_modulo_solars->cantidad }}</span>
                                    @endif
                                </td>
                                <td class="fw-normal align-middle text-center"><button type="button" data-bs-toggle="modal" onclick="modulosolardetalle(this,{{$sede->id}},{{$al_modulo_solars->id_producto}})" data-bs-target="#showmodulo_solar{{$al_modulo_solars->id_producto}}"  class="btn btn-sm btn-secondary"><i class="bi bi-eye-fill text-white"></i></button></td>
                            </tr>
                        @php
                            $contador++;
                        @endphp  
...
@foreach ($al_modulo_solar as $al_modulo_solars)
    @include('ADMINISTRADOR.ALMACEN.inventarios.show_dtllemodulo_solar')
@endforeach
