    ...
    <div class="header_section">
        ...
        <div class="container-fluid">
            <div class="" data-aos="fade-right">
                <h1 class="titulo h2 text-uppercase fw-bold mb-0">PAGOS A PROVEEDORES</h1>
                <div class=""
                    ...
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item link" aria-current="page">Pagos</li>
                    </ol>
                </div>
            </div>
        </div>
                ...
                <div class="card border-0 shadow-sm h-100">
                    <div class="card-body">
                        <div class="d-flex justify-content-between">
                            <div>
                                <p class="text-muted mb-1 small">Pagos Pendientes</p>
                                <h3 class="mb-0 fw-bold text-danger">{{ $ordenesPendientes->count() }}</h3>
                            </div>
            ...
            <div class="col-md-3">
                <div class="card border-0 shadow-sm h-100">
                    <div class="card-body">
                        <div class="d-flex justify-content-between">
                            <div>
                                <p class="text-muted mb-1 small">Total por Pagar</p>
                                <h3 class="mb-0 fw-bold text-danger">S/ {{ number_format($ordenesPendientes->sum('saldo_pendiente'), 2) }}</h3>
                            </div>
                    ...
                    <div class="card-body">
                        <div class="d-flex justify-content-between">
                            <div>
                                ...
                                <h3 class="mb-0 fw-bold text-success">S/ {{ number_format($ordenes->sum('total_pagado'), 2) }}</h3>
                            </div>
            ...
            <div class="col-md-3">
                <div class="card border-0 shadow-sm h-100">
                    <div class="card-body">
                        <div class="d-flex justify-content-between">
                            <div>
                                <p class="text-muted mb-1 small">OC Completadas</p>
                                <h3 class="mb-0 fw-bold text-primary">{{ $ordenesCompletadas->count() }}</h3>
                            </div>
                ...
                <ul class="nav nav-tabs mb-3" id="tabPagos" role="tablist">
                    <li class="nav-item" role="presentation">
                        <button class="nav-link active fw-bold" id="pendientes-tab" data-bs-toggle="tab" data-bs-target="#pendientes" type="button" role="tab">
                            ...
                            <span class="badge bg-danger ms-1">{{ $ordenesPendientes->count() }}</span>
                        </button>
                    </li>
                    <li class="nav-item" role="presentation">
                        <button class="nav-link fw-bold" id="completadas-tab" data-bs-toggle="tab" data-bs-target="#completadas" type="button" role="tab">
                            ...
                            <span class="badge bg-success ms-1">{{ $ordenesCompletadas->count() }}</span>
                        </button>
                    </li>
                </ul>

                <div class="tab-content" id="tabPagosContent">
                    <!-- Tab Pendientes -->
                        ...
                        <table id="tablaPendientes" class="table table-hover align-middle nowrap" cellspacing="0" style="width:100%">
                            <thead class="bg-dark text-white border-0">
                                <tr>
                                    ...
                                    <th class="h6 small text-center text-uppercase fw-bold">Código OC</th>
                                    ...
                                    <th class="h6 small text-center text-uppercase fw-bold">Forma de Pago</th>
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td class="fw-normal text-center align-middle">
                                        <strong>{{ $orden->codigo }}</strong><br>
                                        <small class="text-muted">{{ $orden->created_at->format('d/m/Y') }}</small>
                                    </td>
                                    <td class="fw-normal text-center align-middle">
                                        @if($orden->proveedor)
                                            {{ $orden->proveedor->persona->name ?? 'N/A' }}
                                        @else
                                            N/A
                                        @endif
                                    </td>
                                    <td class="fw-normal text-center align-middle">{{ \Carbon\Carbon::parse($orden->fecha)->format('d/m/Y') }}</td>
                                    <td class="fw-normal text-center align-middle">
                                        @if($orden->forma_pago == 'Crédito')
                                            <span class="badge bg-info">Crédito</span>
                                        @else
                                            <span class="badge bg-secondary">Contado</span>
                                        @endif
                                    </td>
                                    <td class="fw-normal text-center align-middle fw-bold">S/ {{ number_format($orden->total, 2) }}</td>
                                    <td class="fw-normal text-center align-middle fw-bold text-primary">S/ {{ number_format($orden->total_pagado, 2) }}</td>
                                    <td class="fw-normal text-center align-middle fw-bold text-danger">S/ {{ number_format($orden->saldo_pendiente, 2) }}</td>
                                    <td class="fw-normal text-center align-middle">
                                        @php $porcentaje = $orden->total > 0 ? round(($orden->total_pagado / $orden->total) * 100) : 0; @endphp
                                    ...
                                    <td class="text-center align-middle">
                                        <a href="{{ route('admin-pagos.show', $orden) }}" class="btn btn-sm btn-outline-primary" title="Ver Detalle / Registrar Pago">
                        ...
                        <table id="tablaCompletadas" class="table table-hover align-middle nowrap" cellspacing="0" style="width:100%">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td class="fw-normal text-center align-middle fw-bold">S/ {{ number_format($orden->total, 2) }}</td>
                                    <td class="fw-normal text-center align-middle fw-bold text-success">S/ {{ number_format($orden->total_pagado, 2) }}</td>
                                    <td class="text-center align-middle">
                                        <a href="{{ route('admin-pagos.show', $orden) }}" class="btn btn-sm btn-outline-secondary" title="Ver Detalle">
