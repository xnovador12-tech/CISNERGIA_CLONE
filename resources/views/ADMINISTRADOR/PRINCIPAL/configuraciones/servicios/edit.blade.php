<form method="POST" action="{{ route('admin-servicios.update', $admin_servicio->slug) }}" enctype="multipart/form-data" autocomplete="off" class="needs-validation" novalidate>      
    @csrf  
    @method('put')  
    <div class="modal fade" id="editservicios{{ $admin_servicio->slug }}" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
                    ...
                    <div class="row">
                        <div class="col-12 col-md-12 col-lg-12">
                            <div class="mb-3">
                                    ...
                                    <input type="text" name="name" id="name_id" class="form-control form-control-sm @error('name') is-invalid @enderror" value="{{ $admin_servicio->name }}" required maxLength="100">  
                                    @error('name')
                            ...
                            <div class="mb-3">
                                    ...
                                    <textarea class="form-control @error('descripcion') is-invalid @enderror" name="descripcion" placeholder="Escribe una descripción" style="height: 150px">{{ $admin_servicio->descripcion }}</textarea>
                                @error('descripcion')
                                    <small class="text-danger">{{ $message }}</small>
                                @enderror
                            </div>
                            <div class="mb-3">
                                ...
                                <select name="estado" id="estado_id" class="form-select text-uppercase" required>
                                    <option value="{{ $admin_servicio->estado }}" selected="selected" hidden="hidden">{{ $admin_servicio->estado }}</option>
