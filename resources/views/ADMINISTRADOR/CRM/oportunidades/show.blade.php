            ...
            <div class="col-lg-8">
                ...
                <div class="card border-4 borde-top-secondary shadow-sm mb-4" style="border-radius: 20px" data-aos="fade-up">
                    <div class="card-header bg-transparent d-flex justify-content-between align-items-center">
                        ...
                        <a href="{{ route('admin.crm.oportunidades.index') }}" class="btn btn-sm btn-outline-secondary"><i class="bi bi-arrow-left me-1"></i>Volver</a>
                    </div>
                    <div class="card-body">
                        ...
                        @forelse($oportunidad->cotizaciones ?? [] as $cotizacion)
                            <div class="d-flex justify-content-between align-items-center border-bottom pb-2 mb-2">
                                <div>
                                    ...
                                    <span class="badge bg-{{ $cotizacion->estado == 'aceptada' ? 'success' : ($cotizacion->estado == 'rechazada' ? 'danger' : 'warning') }}">{{ ucfirst($cotizacion->estado) }}</span>
                                </div>
                                <div>
            ...
            <div class="col-lg-4">
                ...
                <div class="card border-4 borde-top-secondary shadow-sm mb-4" style="border-radius: 20px" data-aos="fade-up" data-aos-delay="250">
                    ...
                    <div class="card-body">
                        <div class="d-grid gap-2">
                            @if(!in_array($oportunidad->etapa, ['ganada', 'perdida']))
                            <a href="{{ route('admin.crm.oportunidades.edit', ['oportunidad' => $oportunidad, 'redirect_to' => 'show']) }}" class="btn btn-outline-warning btn-sm">
                                <i class="bi bi-pencil me-2"></i>Editar Oportunidad
                            </a>
                            @endif
                            @if($oportunidad->cotizaciones->count() > 0)
                            <a href="{{ route('admin.crm.cotizaciones.show', $oportunidad->cotizaciones->last()) }}" class="btn btn-outline-info btn-sm">
...
$(document).ready(function() {
    ...
    // ==================== AVANZAR ETAPA ====================
    @if(!in_array($oportunidad->etapa, ['ganada', 'perdida']) && $siguienteEtapa)
    $('.btn-avanzar').on('click', function() {
        Swal.fire({
            title: '¿Avanzar de etapa?',
            html: `La oportunidad <strong>{{ $oportunidad->codigo }}</strong> pasará de <strong>{{ $etapaActualInfo['nombre'] }}</strong> a <strong>{{ $siguienteEtapa['nombre'] }}</strong>.
                   <br><br><small class="text-muted">La probabilidad se ajustará a {{ $siguienteEtapa['probabilidad'] }}%</small>`,
            icon: 'question',
            showCancelButton: true,
            confirmButtonColor: '#1C3146',
            cancelButtonColor: '#FF9C00',
            confirmButtonText: '<i class="bi bi-arrow-right me-1"></i> Sí, avanzar a {{ $siguienteEtapa['nombre'] }}',
