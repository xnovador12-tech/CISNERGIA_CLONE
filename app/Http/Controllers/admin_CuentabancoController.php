...
class admin_CuentabancoController extends Controller
{
    ...
    public function edit(Cuentabanco $admin_cuentasbancaria)
    {
        $bancos  = Banco::all();
        $tipos   = Tipocuenta::all();
        $monedas = Moneda::all();
        ...
        return view('ADMINISTRADOR.FINANZAS.cuentasbancarias.edit', compact(
            'admin_cuentasbancaria', 'bancos', 'tipos', 'monedas', 'sedes'
        ));
    }

    public function update(Request $request, Cuentabanco $admin_cuentasbancaria)
    {
        $validated = $request->validate([
            'banco_id'      => 'required|exists:bancos,id',
            'numero_cuenta' => 'required|string|max:255',
        ...
        $admin_cuentasbancaria->update($validated);
